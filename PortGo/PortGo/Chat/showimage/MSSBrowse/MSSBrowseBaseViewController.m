...
- (void)longPress:(MSSBrowseCollectionViewCell *)browseCell
{
    ...
    __weak __typeof(self)weakSelf = self;
    _browseActionSheet = [[MSSBrowseActionSheet alloc]initWithTitleArray:@[NSLocalizedString(@"Save the picture", @"Save the picture")] cancelButtonTitle:NSLocalizedString(@"Cancel",@"Cancel") didSelectedBlock:^(NSInteger index) {
        __strong __typeof(weakSelf)strongSelf = weakSelf;
