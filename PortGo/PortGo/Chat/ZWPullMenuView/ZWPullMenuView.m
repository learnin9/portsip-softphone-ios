...
//三角形
- (void)drawTriangle{
    CGFloat x = CGRectGetMidX(self.anchorRect) - CGRectGetMinX(self.contentView.frame)+10;
    CGFloat y = 0;
    CGPoint p = CGPointZero;
    CGPoint q = CGPointZero;
    //围栏
    if (x < 2 * self.triangleHeight) {
        x = 2 * self.triangleHeight+10;
    }
    if (x > CGRectGetWidth(self.contentView.bounds) - 2 * self.triangleHeight) {
        x = CGRectGetWidth(self.contentView.bounds) - 2 * self.triangleHeight+10;
    }
    //Y中心位置
    //居上：下拉
    //居下：上拉
...
+ (instancetype)pullMenuAnchorView:(UIView *)anchorView
    ...
    menuView.anchorRect = CGRectMake(ScreenWid-95, 0, 100, 80);
