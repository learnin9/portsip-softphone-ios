...
- (void)drawRect:(CGRect)rect{
    ...
    UILabel * descLabel  = [[UILabel alloc] initWithFrame:CGRectMake(0, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 10, rect.size.width, 40)];
    descLabel.font = [UIFont systemFontOfSize:12];
    descLabel.textAlignment = NSTextAlignmentCenter;
    descLabel.textColor = [UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1.0];
    descLabel.text =NSLocalizedString(@"You can start scanning", @"You can start scanning");
    descLabel.numberOfLines = 0;
    
    [self addSubview:descLabel];
    
    UIButton * myCode = [[UIButton alloc] initWithFrame:CGRectMake(0, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 10 + 20 + 10+20, rect.size.width, 20)];
    myCode.titleLabel.font = [UIFont systemFontOfSize:15];
    [myCode setTitle:NSLocalizedString(@"MyQR", @"MyQR") forState:UIControlStateNormal];
