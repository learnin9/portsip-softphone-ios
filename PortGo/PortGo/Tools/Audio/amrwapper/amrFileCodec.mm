...
#if defined(__i386__)||defined(__x86_64__)//模拟器
#ifdef __cplusplus
extern "C" {
#endif
    
    void* Decoder_Interface_init(void){
        NSLog(@"Decoder_Interface_init x86 %@");
        return 0;
    }
    void Decoder_Interface_exit(void* state){
        NSLog(@"Decoder_Interface_exit x86 %@");
        
    }
    void Decoder_Interface_Decode(void* state, const unsigned char* in, short* out, int bfi){
        NSLog(@"Decoder_Interface_Decode x86 %@");
    }
    
    void* Encoder_Interface_init(int dtx){
        NSLog(@"Encoder_Interface_init x86 %@");
        return 0;
    }
    void Encoder_Interface_exit(void* state){
        NSLog(@"Encoder_Interface_exit x86 %@");
    }
    int Encoder_Interface_Encode(void* state, enum Mode mode, const short* speech, unsigned char* out, int forceSpeech){
        NSLog(@"Encoder_Interface_Encode x86 %@");
        return 0;
    }
#ifdef __cplusplus
}
...
// 将AMR文件解码成WAVE文件
float DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename)
{
    #ifdef SUPPORT_AMR
	FILE* fpamr = NULL;
	FILE* fpwave = NULL;
...
//	fpwave = fopen([docFilePath cStringUsingEncoding:NSASCIIStringEncoding], "r+");
	...
	return nFrameCount/AMR_FRAME_COUNT_PER_SECOND;
#else
    return 0;
#endif
}
